...
import kp.ps.exception.PSRuntimeException;
...
public final class PSTuple extends PSValue implements Iterable<PSValue>
{
    ...
    @Override
    public final PSDataType getPSDataType() { return PSDataType.TUPLE; }
    ...
    {
        if(value.getPSDataType() == PSDataType.TUPLE)
        {
    ...
    @Override
    public int resultElementsCount() { return tuple.length; }
    
    @Override
    public PSValue resultElement(int index)
    {
        return index < 0 || index >= tuple.length ? UNDEFINED : tuple[index];
    }
    ...
    private static final PSValue SORT = PSFunction.<PSTuple>method((self, arg0) -> {
        ...
        {
            Arrays.sort(tuple2,(val1, val2) -> {
                return arg0.call(val1,val2).arg0().toJavaInt();
            });
        }
        return new PSTuple(tuple2);
    });
